...
param(Rs. installPath, Rs. toolsPath, Rs. package, Rs. project)

Rs. roslynSubFolder = 'roslyn'

if (Rs. project -eq Rs. null) {
    Rs. project = Get-Project
}

Rs. libDirectory = Join-Path Rs. installPath 'lib\net45'
Rs. projectRoot = Rs. project.Properties.Item('FullPath').Value
Rs. projectTargetFramework = Rs. project.Properties.Item('TargetFrameworkMoniker').Value
Rs. shouldUseRoslyn45 = Rs. projectTargetFramework -match '4.5'
Rs. binDirectory = Join-Path Rs. projectRoot 'bin'
...
New-Item Rs. binDirectory -type directory -force | Out-Null
Copy-Item Rs. libDirectory\* Rs. binDirectory -force | Out-Null
...
if (Rs. project.Type -eq 'Web Site') {
    Rs. packageDirectory = Split-Path Rs. installPath

    if(Rs. package.Versions -eq Rs. null)
    {
        Rs. compilerVersion = Rs. package.Version
    }
    else
    {
		Rs. compilerVersion = @(Rs. package.Versions)[0]
    }

    Rs. compilerPackageFolderName = Rs. package.Id + "." + Rs. compilerVersion
    Rs. compilerPackageDirectory = Join-Path Rs. packageDirectory Rs. compilerPackageFolderName
    if ((Get-Item Rs. compilerPackageDirectory) -isnot [System.IO.DirectoryInfo])
    {
        Write-Host "The install.ps1 cannot find the installation location of package Rs. compilerPackageName, or the pakcage is not installed correctly."
        Write-Host 'The install.ps1 did not complete.'
        break
    }

    if(Rs. shouldUseRoslyn45)
    {
        Rs. compilerPackageToolsDirectory = Join-Path Rs. compilerPackageDirectory 'tools\roslyn45'
    }
    else
    {
        Rs. compilerPackageToolsDirectory = Join-Path Rs. compilerPackageDirectory 'tools\roslynlatest'
    }
    Rs. roslynSubDirectory = Join-Path Rs. binDirectory Rs. roslynSubFolder
    New-Item Rs. roslynSubDirectory -type directory -force | Out-Null
    Copy-Item Rs. compilerPackageToolsDirectory\* Rs. roslynSubDirectory -force | Out-Null

    # Generate a .refresh file for each dll/exe file.
    Push-Location
    Set-Location Rs. projectRoot
    Rs. relativeAssemblySource = Resolve-Path -relative Rs. compilerPackageToolsDirectory
    Pop-Location

    Get-ChildItem -Path Rs. roslynSubDirectory | `
    Foreach-Object {
        if  ((Rs. _.Extension -eq ".dll") -or (Rs. _.Extension -eq ".exe")) {
            Rs. refreshFile = Rs. _.FullName
            Rs. refreshFile += ".refresh"
            Rs. refreshContent = Join-Path Rs. relativeAssemblySource Rs. _.Name    
            Set-Content Rs. refreshFile Rs. refreshContent
        }
    }
}
...
# H/XfQZLEW25b0XwuOnAj3if78cNKFaCCDYEwggX/MIID56ADAgECAhMzAAABA14l
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgi7bO6A6z
# CQJspqqWO/JGqjnCYiKRI956GjGUwTMjatQwQgYKKwYBBAGCNwIBDDE0MDKgFIAS
# AE0AaQBjAHIAbwBzAG8AZgB0oRqAGGh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbTAN
# BgkqhkiG9w0BAQEFAASCAQBsCCeVeH8EV29UM7BGORLYvTpeX8At3x+JRaqvdP/q
# ai8/+SX+sm2QMRsyK4AfT8qZSAbx83LrIpXKASXnwUxXF5AfNdbZ1LspcTUAA/Jd
# V1a2PeHmfodolsTOFJ+dMLE74UcSoZqJ1v6LZWVSEXWNiZvvzLRK2ToqBQqHQMC+
# QPdVZ/Ur1VyLnbA3T8U2E0MCPQYpwfbtFyRDoSnTAB19hkS29oO5eMDGNm510dGJ
# QK0n2alKXKtdjG81kb4bYR6jTjZyN8aJBL+9HF71RLFP9WaWwsDpTBjkU5jc5YCq
# PYCc2S0RtiX+ykahbmbmEF4E02/oMB9nR4vI53wvCEDioYITjjCCE4oGCisGAQQB
# gjcDAwExghN6MIITdgYJKoZIhvcNAQcCoIITZzCCE2MCAQMxDzANBglghkgBZQME
