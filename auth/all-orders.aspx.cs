...
public partial class auth_all_orders : System.Web.UI.Page
{
    ...
    {
        this.rptbindorderdata.DataSource = GetData("SELECT Max(id) as id, Max(order_id) as order_id,Max(order_delivery_time) as order_delivery_time,Max(order_date) as order_date,Max(customer_name) as customer_name,Max(payment_mode) as payment_mode,Max(total_order_amount) as total_order_amount,Max(customer_mobileno) as customer_mobileno FROM ecommerce_order where order_section='Grocery' AND order_date='"+txt_date_from.Text+ "' and order_status!='Cancelled' Group by order_id order by id desc");
        this.rptbindorderdata.DataBind();
    }

    private void BindOrder()
    {
        this.rptbindorderdata.DataSource = GetData("SELECT Max(id) as id, Max(order_id) as order_id,Max(order_delivery_time) as order_delivery_time,Max(order_date) as order_date,Max(customer_name) as customer_name,Max(payment_mode) as payment_mode,Max(total_order_amount) as total_order_amount,Max(customer_mobileno) as customer_mobileno FROM ecommerce_order where order_status='Processing' AND order_section='Grocery' Group by order_id order by id desc");
        this.rptbindorderdata.DataBind();
    }

    private void BindOrderByStatus()
    {
        this.rptbindorderdata.DataSource = GetData("SELECT Max(id) as id, Max(order_id) as order_id,Max(order_delivery_time) as order_delivery_time,Max(order_date) as order_date,Max(customer_name) as customer_name,Max(payment_mode) as payment_mode,Max(total_order_amount) as total_order_amount,Max(customer_mobileno) as customer_mobileno FROM ecommerce_order where order_status='" + dblorderstatus.SelectedValue+ "'  AND order_section='Grocery'  Group by order_id order by id desc");
        this.rptbindorderdata.DataBind();
    }
