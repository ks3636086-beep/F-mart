...
(function () {
  ...
  function updateCountDown(countDownItem) {
    const target = countDownItem.getAttribute('data-count');
    let timeleft = 0;
    ...
    // Fallback: If date passed or invalid, use a rolling 48-hour flash sale
    if (timeleft <= 0) {
      let storedEnd = localStorage.getItem('fmart_flash_sale_timer');
      let endTime = storedEnd ? parseInt(storedEnd, 10) : 0;
      if (!endTime || endTime <= now) {
        endTime = now + (2 * 24 * 60 * 60 * 1000) + (14 * 60 * 60 * 1000) + (36 * 60 * 1000) + (28 * 1000);
        localStorage.setItem('fmart_flash_sale_timer', endTime.toString());
      }
    ...
    if (minutesElement) minutesElement.innerHTML = pad(minutes);
    if (secondsElement) secondsElement.innerHTML = pad(seconds);
  ...
  /**
   * Trending Products Section Slider & Tab Switching
   */
  function initTrendingProducts() {
    ...
    // Initialize Swiper for each pane
    section.querySelectorAll('.trending-swiper').forEach(function (swiperEl) {
      const id = swiperEl.id;
      if (!id || typeof Swiper === 'undefined') return;
    ...
    // Handle Tab Buttons
    const tabBtns = section.querySelectorAll('.trend-tab-btn');
    const panes = section.querySelectorAll('.trending-tab-pane');
      ...
      btn.addEventListener('click', function () {
        ...
        panes.forEach(function (pane) {
          if ('#' + pane.id === targetId) {
            ...
            const swiperEl = pane.querySelector('.trending-swiper');
            if (swiperEl && swiperInstances[swiperEl.id]) {
              swiperInstances[swiperEl.id].update();
            }
          } else {
    ...
    // Handle Shared Prev/Next Navigation
    const prevBtn = section.querySelector('.trend-btn-prev');
    const nextBtn = section.querySelector('.trend-btn-next');

    function getActiveSwiper() {
      const activePane = section.querySelector('.trending-tab-pane.active');
      if (!activePane) return null;
      const swiperEl = activePane.querySelector('.trending-swiper');
      if (swiperEl && swiperInstances[swiperEl.id]) {
        return swiperInstances[swiperEl.id];
      }
      return null;
    }
  ...
  window.addEventListener('load', initTrendingProducts);

  /**
   * Top Picks for You Deals Swiper & Wishlist Buttons
   */
  function initCtaDealsSwiper() {
    ...
    const prevBtn = document.querySelector('.cta-arrow-prev');
    const nextBtn = document.querySelector('.cta-arrow-next');
    ...
    document.querySelectorAll('.deal-wish-btn').forEach(function (btn) {
